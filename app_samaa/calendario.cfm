			...
			<tr>
				<cfloop index ="j" from="1" to="7">
					...
					<cfif #theDay# GT 0 AND #theDay# LT #vLimiteDias#>
						...
						<cfquery name="tbSesionPleno" datasource="#vOrigenDatosSAMAA#">
							SELECT * FROM sesiones 
                            WHERE ssn_clave = 1 AND (year(ssn_fecha) = year('#vDiaHoy#') AND month(ssn_fecha) = month('#vDiaHoy#') AND day(ssn_fecha) = day('#vDiaHoy#'))
						</cfquery>
						<cfquery name="tbSesionExtra" datasource="#vOrigenDatosSAMAA#">
							SELECT * FROM sesiones WHERE ssn_clave = 2 AND (year(ssn_fecha) = year('#vDiaHoy#') AND month(ssn_fecha) = month('#vDiaHoy#') AND day(ssn_fecha) = day('#vDiaHoy#'))
						</cfquery>
						<cfquery name="tbSesionCAAA" datasource="#vOrigenDatosSAMAA#">
							SELECT * FROM sesiones WHERE ssn_clave = 4 AND (year(ssn_fecha) = year('#vDiaHoy#') AND month(ssn_fecha) = month('#vDiaHoy#') AND day(ssn_fecha) = day('#vDiaHoy#'))
						</cfquery>
						<cfquery name="tbSesionDoc" datasource="#vOrigenDatosSAMAA#">
							SELECT * FROM sesiones WHERE ssn_clave = 5 AND (year(ssn_fecha) = year('#vDiaHoy#') AND month(ssn_fecha) = month('#vDiaHoy#') AND day(ssn_fecha) = day('#vDiaHoy#'))
						</cfquery>
						
						<cfif #val(theDay)# EQ #val(LsDateFormat(tbSesionPleno.ssn_fecha,"d"))#>
							<cfset vBgrd = "5EBEFF">
							<cfset vSesionPlenoIndica = "Sesi&oacute;n Ordinaria del Pleno (" & #LsNumberFormat(tbSesionPleno.ssn_id,"9999")# & ") a partir de las " & #LsTimeFormat(tbSesionPleno.ssn_fecha,"HH:mm")# & " hrs.">
						<cfelseif #val(theDay)# EQ #val(LsDateFormat(tbSesionExtra.ssn_fecha,"d"))#>
							<cfset vBgrd = "B3E0FF">
							<cfset vSesionPlenoIndica = "Sesi&oacute;n Extraordinaria del Pleno a partir de las " & #LsTimeFormat(tbSesionExtra.ssn_fecha,"HH:mm")# & " hrs.">
						<cfelseif #val(theDay)# EQ #val(LsDateFormat(tbSesionCAAA.ssn_fecha,"d"))#>
							<cfset vBgrd = "FF9900">
							<cfset vSesionPlenoIndica = "Reuni&oacute;n de la CAAA: " & #LsNumberFormat(tbSesionCAAA.ssn_id,"9999")#>
						<cfelseif #val(theDay)# EQ #val(LsDateFormat(tbSesionDoc.ssn_fecha,"d"))#>
							<cfset vBgrd = "CCCCCC">
							<cfset vSesionPlenoIndica = "Recepci&oacute;n de documentos para sesi&oacute;n: " & #LsNumberFormat(tbSesionDoc.ssn_id,"9999")#>
						<cfelse>
							<cfset vBgrd = "FCFAE9">
						...
						<cfif NOT (theDay LT 1 OR theDay GT Session.nDays)>
							<cfif trigger eq 1>
								<cfif #vSesionPlenoIndica# NEQ ''>
									<span class="Sans11NeNe" title="#vSesionPlenoIndica#" style="cursor:pointer;">#theDay#</span>
								<cfelse>
									<span class="Sans11Ne" title="#vSesionPlenoIndica#">#theDay#</span>                                
								</cfif>
							</cfif>
