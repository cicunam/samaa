	...
	<cfelseif #vAccion# EQ "D">
		...
		<cfoutput query="tbSesionOrden">
			<cfquery datasource="#vOrigenDatosSAMAA#">
				...
				SET punto_num = #C#
				WHERE ssn_id = #vSsnId# AND punto_num = #tbSesionOrden.punto_num#
			</cfquery>	
			<cfset C = C + 1>
		</cfoutput>
	</cfif>
	<cfif #vAccion# EQ "S">
		...
		<cfif #vPunto# GT 1>
			<cfquery datasource="#vOrigenDatosSAMAA#">
				UPDATE sesiones_orden 
				SET punto_num = 99
				WHERE ssn_id = #vSsnId# AND punto_num = #vPunto#
			</cfquery>
			<cfquery datasource="#vOrigenDatosSAMAA#">
				UPDATE sesiones_orden 
				SET punto_num = #vPunto#
				WHERE ssn_id = #vSsnId# AND punto_num = #vPunto# - 1
			</cfquery>
			<cfquery datasource="#vOrigenDatosSAMAA#">
				UPDATE sesiones_orden 
				SET punto_num = #vPunto# - 1
				WHERE ssn_id = #vSsnId# AND punto_num = 99
			</cfquery>
		</cfif>	
	</cfif>
	<cfif #vAccion# EQ "B">
		...
		<cfquery name="tbSesionOrden" datasource="#vOrigenDatosSAMAA#">
			SELECT * FROM sesiones_orden 
			WHERE ssn_id = #vSsnId# ORDER BY punto_num
		</cfquery>
	...
	<cfif (#vAccion# EQ "N" OR #vAccion# EQ "E")>
		...
		<cfif IsDefined("punto_pdf") AND #punto_pdf# IS NOT ''>
			...
			<cfif #punto_clave# IS 'ACTA'>
				<cfset vNombreArchivo = 'ACTA_' & #vSsnId#-1 & '.pdf'> 
			<cfelseif  #punto_clave# IS 'ACTAS'>
				<cfset vNombreArchivo = 'ACTA_' & #vSsnId#-2 & '.pdf'> 
			<cfelseif  #punto_clave# IS 'RECOMCAAA'>
				<cfset vNombreArchivo = 'RECOMCAAA_' & #vSsnId# & '.pdf'>
			<cfelseif  #punto_clave# IS 'RECOMCAAAS'>
				<cfset vNombreArchivo = 'RECOMCAAA_' & #vSsnId#-1 & '.pdf'>
			<cfelseif  #Rtrim(punto_clave)# IS 'PNCA' OR #Rtrim(punto_clave)# IS 'PNIE'>
				<cfset vNombreArchivo = #vSelAcad# & "_" & #punto_clave# & "_" & #vSsnId# & '.pdf'>
			<cfelse>
		...
		<cfif IsDefined("punto_pdf_2") AND #punto_pdf_2# IS NOT ''>
			...
			<cfif #punto_clave# IS 'ACTAS'>
				<cfset vNombreArchivo2 = 'ACTA_' & #vSsnId#-1 & '.pdf'> 
			...
			<cfelseif #punto_clave# IS 'RECOMCAAAS'>
				<cfset vNombreArchivo2 = 'RECOMCAAA_' & #vSsnId# & '.pdf'>
			<cfelse>	
