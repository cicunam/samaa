...
<cfquery name="tbMenus" datasource="#vOrigenDatosSAMAA#">
    <cfif #Session.sTipoSistema# EQ 'sic'>
		SELECT menu_clave, menu_nombre, menu_descrip_sic AS menu_descrip, ruta_liga_cic AS ruta_liga  
        FROM samaa_menu
		WHERE 1 = 1
    	AND menu_activo_sic = 1
		AND nivel_acceso_sic LIKE '-%#Session.sUsuarioNivel#%-'
	<cfelseif #Session.sTipoSistema# EQ 'stctic'>   <!--- ANTES 'stctic' --->
		SELECT menu_clave, menu_nombre, menu_descrip_cic AS menu_descrip,
    	<cfif Find("app_samaa", CGI.SCRIPT_NAME) GT 0>
			menu_liga_modulo
		<cfelse>
			ruta_liga_cic 
		</cfif>
