	...
	<cfif IsDefined("vTipoComando") AND #vTipoComando# EQ "NUEVO">
		...
		<cfquery datasource="#vOrigenDatosSAMAA#">
			...
			<cfif IsDefined("vSesionId") AND #vSesionId# NEQ "">
...
<!-- GUARDA LOS CAMBIOS DEL REGISTRO EDITADO-->
	...
	<!-- GUARDA EL REGISTRO DE RECEPCION DE DOCUMENTOS-->
		<cfquery datasource="#vOrigenDatosSAMAA#">
			...
			WHERE ssn_clave = 5 AND ssn_id = #vSesionId#
		</cfquery>
	...
	<!-- GUARDA EL REGISTRO PARA LA COMISIÓN DE ASUNTOS ACADÉMICO ADAMINISTRATIVOS-->
		<cfquery datasource="#vOrigenDatosSAMAA#">
			UPDATE sesiones SET 
				...
				,
				cap_fecha_mod = GETDATE()
			WHERE ssn_clave = 4 AND ssn_id = #vSesionId#
		</cfquery>
	...
	<!-- GUARDA EL REGISTRO DE ENVÍO DE CORRESPONDENCIA-->
		<cfquery datasource="#vOrigenDatosSAMAA#">
			UPDATE sesiones SET
				...
				,
				cap_fecha_mod = GETDATE() 
			WHERE ssn_clave = 3 AND ssn_id = #vSesionId#
		</cfquery>
	...
	<!-- GUARDA EL REGISTRO PARA SESION ORDINARIA DEL PLENO-->
		<cfquery datasource="#vOrigenDatosSAMAA#">
			...
			<cfif IsDefined("ssn_notaPleno") AND #ssn_notaPleno# NEQ "">
				...
				,
				cap_fecha_mod = GETDATE() 
			WHERE ssn_clave = 1 AND ssn_id = #vSesionId#
		</cfquery>
	</cfif>
	
	<cflocation url="sesion_ordinaria.cfm?vSesionId=#vSesionId#&vTipoComando=CONSULTA" addtoken="no">
