...
<!--- Obtener datos del catálogo de categorías y niveles (CATÁLOGOS GENERALES MYSQL) --->
<cfquery name="tbSolicitudesCoa" datasource="#vOrigenDatosSOLCOA#">
	SELECT *
<!---    
    , 
        (
            SELECT COUNT(*) AS vOponente 
            FROM SAMAA.dbo.convocatorias_coa_concursa 
            WHERE coa_id = <cfif #vTipoComando# EQ 'NUEVO'>'#vIdCoa#'<cfelse>'#vCampoPos23#'</cfif>
            AND solicitud_id_coa = solicitudes.solicitud_id
        )
--->
    FROM solicitudes
    WHERE coa_id = <cfif #vTipoComando# EQ 'NUEVO'>'#vIdCoa#'<cfelse>'#vCampoPos23#'</cfif>
    AND solicitud_status BETWEEN 5 AND 6 
...
--->
                        ...
                        <table border="0" class="cuadrosFormularios">
                            <tr>
                                <td><div id="oponente_dynamic"><!-- AJAX: Lista oponentes --></div></td>
                            </tr>
                            <cfif #vTipoComando# IS 'EDITA'>                            
                                <tr>
                                    <td><div id="solicitudcoa_dynamic"><!-- AJAX: Lista solicitudes COA --></div></td>
                                </tr>
                            </cfif>
                        </table>
