...
<cfquery name="tbMovimientos" datasource="#vOrigenDatosSAMAA#">
	...
	AND C2.dec_super = 'DE' <!--- Asuntos devueltos SE CAMBIO POR EL CÓDIGO DE ABAJO --->
	<!--- AND movimientos_asunto.dec_clave = 10  Asuntos devueltos --->
	<!--- Creo que aquí falta la validación de no más de 15 días --->
	...
	<head>
		...
		<script type="text/javascript">
			...
			function fValidaCamposFt()
			{
				...
				vMensaje += <cfoutput>#tbMovimientos.RecordCount#</cfoutput> > 0 ? '' : 'No se encontró el dictamen de DEFINITIVIDAD devuelto por el CTIC.\n';
...
<script type="text/javascript">
	<cfif #vTipoComando# EQ 'NUEVO' AND #tbMovimientos.RecordCount# LT 1>
		alert('AVISO: No se encontró el dictamen de DEFINITIVIDAD devuelto por el CTIC. Si prosigue con la captura la solicitud será rechazada.');
	</cfif>
</script>
