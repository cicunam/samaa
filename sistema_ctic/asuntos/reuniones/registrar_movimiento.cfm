...
<cfquery name="tbSolicitudes" datasource="#vOrigenDatosSAMAA#">
    SELECT * FROM consulta_solicitud_registraMov
	WHERE ssn_id = #vActa#
	AND 
    (
		(
            asu_oficio IS NOT NULL 
            AND mov_clave <> 
            14 AND sol_status = 0) <!--- NO ENVIAR ASUNTOS SIN NUMERO DE OFICIO --->
		OR 
		(mov_clave = 14) <!--- BAJAS NO REQUIEREN NÚMERO DE OFICIO --->
	)
	ORDER BY
	asu_parte,
	asu_numero

<!--- ELIMINAR, SE SUSTITUYE POR UNA UNA CONSULTA 14/11/2022
    ...
    (
		(
            T2.asu_oficio IS NOT NULL 
            AND T1.mov_clave <> 
            14 AND T1.sol_status = 0) <!--- NO ENVIAR ASUNTOS SIN NUMERO DE OFICIO --->
		OR 
		(T1.mov_clave = 14) <!--- BAJAS NO REQUIEREN NÚMERO DE OFICIO --->
	)
	ORDER BY
	T2.asu_parte,
	T2.asu_numero
    ...
    <cfoutput query="tbSolicitudes">
        ...
        <cfif #tbMovimientosVerifica.RecordCount# IS 0>
            ...
            <cfelseif #tbSolicitudes.dec_super# IS 'OB' OR #tbSolicitudes.dec_super# IS 'PE'>
                <cfset vActaPosterior= #vActa# + 1>
                <!--- Verifica que no existan los registros de la sesión a asignar para evitar duplicidades  --->
                <cfquery name="tbMovAsuntoCoteja" datasource="#vOrigenDatosSAMAA#">
                    SELECT COUNT(*) AS vCuenta 
                    FROM movimientos_asunto
                    WHERE sol_id = #tbSolicitudes.sol_id#
                    AND ssn_id = #vActaPosterior#
                </cfquery>
                <!--- Si no existen registro en la sesión hace el procedimiento --->
                <cfif #tbMovAsuntoCoteja.vCuenta# EQ 0>
                    <cfif #tbSolicitudes.dec_clave# IS 24>
                        <cfset vAsuReunion = 'CTIC'>
                        <cfset vSolStatus = 1>
                        <cfset vSolOficio = '#tbSolicitudes.asu_oficio#'>
                    <cfelse>
                        <cfset vAsuReunion = 'CAAA'>
                        <cfset vSolStatus = 2>
                        <cfset vSolOficio = 'NULL'>
                    </cfif>
                    ...
                    <cfquery datasource="#vOrigenDatosSAMAA#">
                        ...
                        VALUES (#tbSolicitudes.sol_id#, #vActaPosterior#, '#vAsuReunion#', #ReasignarSeccionEnListado(tbSolicitudes.dec_super, tbSolicitudes.asu_parte)#, #Iif(tbSolicitudes.asu_numero IS '',0,tbSolicitudes.asu_numero)#, '#vSolOficio#')
                    </cfquery>
                    <!--- Cambiar el status de la solicitud --->
                    <cfquery datasource="#vOrigenDatosSAMAA#">
                        UPDATE movimientos_solicitud 
                        SET sol_status = #vSolStatus#
                        WHERE sol_id = #tbSolicitudes.sol_id#
                    </cfquery>
            ...
            <cfelse>
                ...
                <cfif #tbSolicitudes.mov_clave# IS NOT 31>
                    ...
                    <cfquery name="tbSesiones" datasource="#vOrigenDatosSAMAA#">
                        SELECT * FROM sesiones WHERE ssn_clave = 1 AND ssn_id = #vActa#
                    </cfquery>
                    ...
                    <cfquery datasource="#vOrigenDatosSAMAA#">
                        ...
                        <cfelseif #tbSolicitudes.mov_clave# IS 17 OR #tbSolicitudes.mov_clave# IS 28><!--- Movimientos que inicina un día un día despues de la reunión de pleno --->
                            <!--- SE ELIMINÓ ESTE CÓDIGO YA QUE EL USO DE LA PLAZA ES AL FINALIZAR EL ÚLTIMO CONTRATO DEL PROPIETARIO DE LA PALZA (#tbSolicitudes.mov_clave# IS 5 AND #tbSolicitudes.sol_pos17# EQ 1) Sólo los COA sin oponente se le asigna como fecha de inicio el día después de la sesión del pleno --->
                            <cfif #tbSesiones.ssn_fecha_m# IS ''><!--- Si no cambió la fecha de la sesión --->
                        ...
                        ,
                        <!--- mov_fecha_final --->
                        <cfif #tbSolicitudes.mov_clave# IS 17 AND #tbSolicitudes.sol_pos5# IS NOT 1><!--- Si el movimiento no es para personal interino --->
                            <cfif #tbSesiones.ssn_fecha_m# IS ''>
                                '#LsDateFormat(DateAdd("yyyy",1,tbSesiones.ssn_fecha)-1,"dd/mm/yyyy")#' <!--- Se agregó un -1 ya agrega un día más en la fecha final 19/09/2023 --->
                            <cfelse>
                                 '#LsDateFormat(DateAdd("yyyy",1,tbSesiones.ssn_fecha_m)-1,"dd/mm/yyyy")#' <!--- Se agregó un -1 ya agrega un día más en la fecha final 19/09/2023 --->
                            </cfif>
                        <cfelseif #tbSolicitudes.mov_clave# IS 5 ><!--- Para COA, se separó del IF de arriba para poner la fecha del campo sol_pos14 + 1 año (05/09/2018)--->
                             '#LsDateFormat(DateAdd("yyyy",1,tbSolicitudes.sol_pos14)-1,"dd/mm/yyyy")#' <!--- Se agregó un -1 ya agrega un día más en la fecha final 19/09/2023 --->
...
<cfoutput>
	<div class="Sans10Ne" style="width:800px; margin: 10px 0px 10px 15px; border: none">
		<span class="Sans11NeNe" >Informe de registro de solictitudes a movimientos del ACTA #vActa#:<br></span>
		<hr>
		<ul>
			<li>Se registraron <span class="Sans10NeNe">#cRegistrados#</span> movimientos acad&eacute;mico-administrativos.</li>
			<li>Se pasaron <span class="Sans10NeNe">#cPendientes#</span> asuntos pendientes a la sesi&oacute;n <span class="Sans10NeNe">#Val(vActa) + 1#</span>.</li>
			<li>Se registraron <span class="Sans10NeNe">#cCorrecciones#</span> correcciones a oficio.</li>
			<li>Se retiraron <span class="Sans10NeNe">#cRetirados#</span> asuntos.</li>
		</ul>
		<hr>
...
<cffunction name="ReasignarSeccionEnListado">
	...
	<cfif #aSD# IS 'OB'><!--- Objetados --->
		...
		<cfelseif Val(#aPT#) EQ 3 OR Val(#aPT#) EQ 2.2 OR Val(#aPT#) EQ 4.2>
			<cfreturn 4.2>
		<cfelseif Val(#aPT#) EQ 3.3><!--- AGREGUÉ ESTE CÓDIGO PARA LOS OBJETADOS DE LA SECCIÓN III.III 06/10/2022 --->
			<cfreturn 4.3>
		<cfelseif Val(#aPT#) EQ 3.4><!--- AGREGUÉ ESTE CÓDIGO PARA LOS OBJETADOS DE CÁTEDRAS CONACyT 12/06/2015 --->
			<cfreturn 4.4>
		<cfelseif Val(#aPT#) EQ 3.1><!--- AGREGUÉ ESTE CÓDIGO PARA LOS OBJETADOS DE BECAS POSDOCTORALES 19/04/2024 --->
			<cfreturn 4.3>
<!---   PENDIENTE POR LIBERAR 14/11/2022
        <cfelse>
			<cfreturn NULL>
--->
		</cfif>
	<cfelseif #aSD# IS 'PE'><!--- Pendientes por ausencia --->
		...
		<cfelseif Val(#aPT#) EQ 4.2>
			<cfreturn 4.2>
		<cfelseif Val(#aPT#) EQ 2.5 OR Val(#aPT#) EQ 5><!--- SE AGREGÓ EL 21/10/2022 Y SE MODIFICÓ EL 17/04/2023 --->
			<cfreturn 2.5>
<!---   PENDIENTE POR LIBERAR 14/11/2022
        <cfelse>
			<cfreturn 7>
