...
<!--- FECHA ÚLTIMA MOD: 01/02/2022 --->

<!--- PRIMERA VERSIÓN, COPIA DEL 24/05/2022--->
	...
	<head>
		...
		<script language="JavaScript" type="text/JavaScript">
			...
			function fGenerarNoOficios(vTipo)
			{
				...
				{
					fGenerarNoOficiosAjax('RES');
				}
				// Desmarcar todas las solicitudes y borrar el número inicial:
				fMarcarTodas(false);
				document.getElementById('vNoOficio').value = '';
				alert('Se asignaron correctamente los números de oficio');				
			}

			// Asignar números de oficio a los sabáticos con beca DGAPA a enviar a la misma DGAPA:
			function fGenerarNoOficiosDgapa()
			{
				// Verificar que se haya asignado el número inicial:
				if (isNaN(parseInt(document.getElementById('vNoOficioDGAPA').value)) || parseInt(document.getElementById('vNoOficioDGAPA').value) == 0)
				{
					alert('Debe indicar el número de oficio a partir del cual se generarán los oficios de respuesta.');
					return;
				}
				else
				{
					if (document.getElementById('hiddRegMarcados').value > 0)
					{
						fGenerarNoOficiosAjax('OFDGAPA');
						fMarcarTodas(false);
						document.getElementById('vNoOficioDGAPA').value = '';
						alert('Se asignaron correctamente los números de oficio para la DGAPA');
					}
					else
					{
						alert('Debe seleccionar las solicitudes a asingnar número de oficio.');
						return;
					}
				}
			}
			...
			function fGenerarNoOficiosAjax(vTipo)
			{
				...
				parametros = "vTipo=" + encodeURIComponent(vTipo);
				parametros += "&vActa=" + encodeURIComponent(document.getElementById('vActa').value);
				if (vTipo == 'RES' || vTipo == 'LYC')
					parametros += "&vNoOficio=" + encodeURIComponent(document.getElementById('vNoOficio').value);
				if (vTipo == 'OFDGAPA')
					parametros += "&vNoOficio=" + encodeURIComponent(document.getElementById('vNoOficioDGAPA').value);				
				// Enviar la petición HTTP:
				xmlHttp.send(parametros);
			}
			...
			<tr>
				...
				<td width="180" valign="top" class="bordesmenu">
					...
					<table width="180" border="0">
						...
						<tr>
							<td><input type="button" value="Aprobar/Ratificar" class="botones" onClick="fAsignarDecision();"></td>
						</tr>
						...
						<tr>
							<td><input type="button" value="Imprimir" class="botones" onClick="fImprimirListado();"></td>
						</tr>
						<!-- Asignar números de oficio -->
						<tr><td><br><div class="linea_menu"></div></td></tr>
						<tr>
						...
						<!-- Tipo de oficios -->
						<tr>
							<td><span class="Sans9ViNe" align="justify">Asignar n&uacute;mero de oficio a los asuntos seleccionados.</span></td>
						</tr>
						...
						<!-- Asignar números de oficio a sabáticos con beca DGAPA -->
						<!-- Tipo de oficios -->
						<tr>
							<td><span class="Sans9ViNe" align="justify">Asignar n&uacute;mero de oficio a los sabáticos con beca DGAPA.</span></td>
						</tr>
						<!-- A partir del número de oficio --->
						<tr>
							<td>
								<span class="Sans9GrNe">A partir del No.</span>
								<input name="vNoOficioDGAPA" id="vNoOficioDGAPA" type="text" maxlength="6" style="width: 40px;" class="datos">
							</td>
						</tr>
						<!-- Registrar movimientos: Botón de comando -->
						<tr>
							<td>
								<input type="button" value="Asignar" class="botones" onClick="fGenerarNoOficiosDgapa();">
							</td>
						</tr>                        
...
--->						                        
						...
						<tr id="trRegMovCmd" style="display:none;">
							<td>
								<input type="button" value="Registrar" class="botones" onClick="fRegistrarMovimientos();">
							</td>
						</tr>
						<!---
						<!-- Navegación -->
